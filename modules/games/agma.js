...
class Bot {
	...
	spawn() {
        if (!this.agma.readyToSpawn) return;
		let name = getConfig().botNames[Math.floor(Math.random() * getConfig().botNames.length)] + ' | ' + Math.random().toString(36).slice(2);
...
class Agma {
    ...
    get array() {
        var o = "~9B\\x$";
        return [o.charCodeAt(0), o.charCodeAt(1), o.charCodeAt(2) + 73, o.charCodeAt(3), o.charCodeAt(4) + 227, o.charCodeAt(5)];
    }

    getCookie(proxy) {
        return new Promise(async (resolve, reject) => {
            let jar = request.jar();
            request.get('https://agma.io/', {
                jar,
                gzip: true,
                headers: {
                    'authority': 'agma.io',
                ...
                request.post('https://agma.io/client.php', {
                    jar,
                    gzip: true,
                    headers: {
                    ...
                    for (var i = 0; i < Agma.emgaa.length; i++)
                        this.numChars += (Agma.emgaa.charCodeAt(i) * (1 - (!i || i % 2 ? 0 : 2)) - 1 * (i ? 0 : 1));
                    --this.numChars;
                    resolve(cookie);
                });
            });
            ...
            if (5 < num.length) {
                ...
                if (!isNaN(right) && !isNaN(date)) {
                    ...
                    for (var i = 0; i < right.length; i++)
                        checkVal = checkVal + (parseInt(right.substr(i, 1)) + 30 + vkx.charCodeAt(1) + blockGap) * (i + 1);
    ...
    round(buffer, offset, length, n) {
        ...
        var i = 0;
        for (; length > i; i++)
            s = s + buffer.readUInt8(offset + i) * (i + 1);
        return s;
    }
    ...
    replace(selector) {
        ...
        buffer.writeUInt8((2 * (this.selector + 30) - (this.getWidth - 5) % 10) - 5);
        buffer.writeUInt32LE(~~(this.getWidth / 1.84 + (this.selector / 2) - 2 * (selector ? .5 : 1)) + (~~(~~(21.22 * (~~(this.getWidth + 4.42 * this.chunkOffset + 555) % --this.label + 36360)) / 4.2)), 1);
    ...
    get sortDataByVersion() {
        ...
        for (; i < this.array.length; i++)
            source = source + ~~(this.getWidth / this.array[i] - this.array[i] % this.numChars);
        return source;
    }
